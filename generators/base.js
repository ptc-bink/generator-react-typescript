...
class BaseGenerator extends YO.Base {
    ...
    _writeApp() {
        const src = this.config.get('src');
        this.fs.copyTpl(this.templatePath('app.tsx.ejs'), this.destinationPath(`${src}/app.tsx`), {
            appname: this.appname
        });
    }
    _writeComponents() {
        const src = this.config.get('src');
        this.fs.copyTpl(this.templatePath('components/index.ts.ejs'), this.destinationPath(`${src}/components/index.ts`), {
            appname: this.appname
        });
    }
    _writeContainers() {
        const src = this.config.get('src');
        this.fs.copyTpl(this.templatePath('containers/index.ts.ejs'), this.destinationPath(`${src}/containers/index.ts`), {
            appname: this.appname
        });
    }
    _writeComponent(name) {
        console.log("qqq");
        const src = this.config.get('src');
        this.fs.copyTpl(this.templatePath('component.tsx.ejs'), this.destinationPath(src, `components/${name}/index.tsx`), {
            name: name
        });
    }
    _writeThemes(component) {
        ...
        this.fs.copyTpl(this.templatePath('theme.scss.ejs'), this.destinationPath(src, `components/${component}/theme.scss`), {
            theme: 'default',
            component: component
        });
        themes.forEach(theme => {
            this.fs.copyTpl(this.templatePath('theme.scss.ejs'), this.destinationPath(src, `components/${component}/theme-${theme}.scss`), {
                theme: theme,
                component: component
            });
        });
    ...
    _writeContainer(name) {
        const src = this.config.get('src');
        this.fs.copyTpl(this.templatePath('container.tsx.ejs'), this.destinationPath(src, `containers/${name}.tsx`), { name: name });
    }
    _writeComponentsIndex(components) {
        const src = this.config.get('src');
        this.fs.copyTpl(this.templatePath('index.ts.ejs'), this.destinationPath(src, 'components/index.ts'), {
    ...
    _writeContainersIndex(containers) {
        ...
        this.fs.copyTpl(this.templatePath('index.ts.ejs'), this.destinationPath(src, 'containers/index.ts'), {
            containers: containers.sort()
        });
    }
    _writeTheme(theme, components = this.getComponents()) {
        ...
        components.forEach(component => {
            this.fs.copyTpl(this.templatePath('theme.scss.ejs'), this.destinationPath(src, `components/${component}/theme-${theme}.scss`), { theme: theme, component: component });
            this.fs.copyTpl(this.templatePath('style.scss.ejs'), this.destinationPath(src, `components/${component}/style.scss`), { themes: themes, component: component });
        });
    }
